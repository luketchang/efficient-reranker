...
class MSEMarginLoss(nn.Module):
    ...
    def forward(self, positive_scores, negative_scores, positive_labels, negative_labels):
        ...
        num_negatives = len(negative_scores) // len(positive_scores)
        positive_scores_repeated = positive_scores.repeat_interleave(num_negatives)
        positive_labels_repeated = positive_labels.repeat_interleave(num_negatives)
...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, positive_scores, negative_scores, temperature=0.1):
        """
